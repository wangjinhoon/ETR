...
void VRTrackerReader::getData(TrackingData& data, uint32_t identifier, std::ofstream& outputFile, float* ini_x, float* ini_y, float* ini_z, bool a) {
    ...
    //initilization
    if (identifier == 6 && *ini_x == 0.0 && *ini_y == 0.0 && *ini_z == 0.0 && a) {
        *ini_x = devicePose.mDeviceToAbsoluteTracking.m[0][3] * METERTOUNREALUNITS;
        *ini_y = devicePose.mDeviceToAbsoluteTracking.m[1][3] * METERTOUNREALUNITS;
        *ini_z = -devicePose.mDeviceToAbsoluteTracking.m[2][3] * METERTOUNREALUNITS;
        //outputFile << " *ini_x : " << *ini_x
    ...
    if (identifier == 6 && a) {
        ...
        // Convert microseconds to seconds and fractions of a second
        ...
        std::cout << seconds.time_since_epoch().count() << "."
            << fractional_seconds.count() << std::endl;
        ...
        outputFile << seconds.time_since_epoch().count() << "."
                   ...
                   << " " << (devicePose.mDeviceToAbsoluteTracking.m[0][3] * METERTOUNREALUNITS - *ini_x)/100
                   << " " << -(devicePose.mDeviceToAbsoluteTracking.m[1][3] * METERTOUNREALUNITS - *ini_y)/100
                   << " " << -(devicePose.mDeviceToAbsoluteTracking.m[2][3] * METERTOUNREALUNITS - *ini_z)/100
                   << " " << orientation.x
                   << " " << orientation.y
                   << " " << orientation.z
...
void VRTrackerReader::run() {
    ...
    while (true) {
        ...
        for (auto& openVRIdAndUnrealId : openVRIDAndUnrealIDs) {
            ...
            // 실행 시간 출력
            std::cout << "함수 실행 시간: " << elapsed_time.count() << " 초" << std::endl;
            sender.sendData(openVRIdAndUnrealId.unrealID, data);
        }
    }
}
